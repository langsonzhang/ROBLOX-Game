...
function cooldown.new(timeframe)
	...
	--		self.cache[tostring(player.UserId)] = os.clock()
	--	end
	--end
...
function cooldown:add()
	local handle = http:GenerateGUID(false)
	self.cache[handle] = os.clock()
	return handle
end

function cooldown:addReduced(lowerBound: number)
	local handle = http:GenerateGUID(false)
	self.cache[handle] = os.clock() - lowerBound
	return handle
end
...
function cooldown:IsReady(userId)
	...
	if last then 
		if os.clock() - last < self.time then 
			return false 
		end
	else
		return false
	...
	self.cache[tostring(userId)] = os.clock() -- automatically reset 
	
	return true 
	
end

function cooldown:reset(id)
	self.cache[id] = os.clock()
end

function cooldown:timeLeft(userId)
	local last = self.cache[tostring(userId)]

	if last then 
		return last + self.time - os.clock()
	end
	return -1
end

function cooldown:Clear(id: string)
